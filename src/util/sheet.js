...
const Sheet = function(sheetReference) {
  ...
  (function() {
    var matches = sheetReference.match(
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          return callback(
            new SheetNotFoundError(ExceptionMessages.SHEET_NOT_FOUND)
          );
        } else {
          return callback(
            new UnauthorizedError(ExceptionMessages.UNAUTHORIZED)
          );
        }
      }
    };
  ...
  function processSheetData(
    ...
    self
      ...
      .then(r =>
        createBlips(
          sheetResponse.result.properties.title,
          r.result.values,
          sheetNames
        )
      )
